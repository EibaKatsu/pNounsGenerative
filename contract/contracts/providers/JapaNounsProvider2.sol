...
contract JapaNounsProvider2 is IAssetProviderEx, Ownable, IERC165 {
  ...
  function background(uint256 _assetId) internal pure returns (SVG.Element memory) {
    ...
    while (numOfElements < heights.length) {
      ...
      if (numOfElements == heights.length - 1 || totalY + height >= 1024) {
    ...
    for (uint256 i = 0; i < numOfElements; i++) {
      ...
      SVG.Element memory ele = SVG.rect(0, int256(stack.y), 1024, heights[i]).fill(colors[stack.color]);
      elements[i] = ele;
    }
  ...
  function hands() public pure returns (SVG.Element memory) {
    bytes memory hand = 'M1 0h1v2h1V1h1V0h1v1H4v1h2V1h1v1H6v1H5v1h2v1H6v1H5V5H3v1H0V2h1z';
    return
      SVG.list(
        [
          SVG.path(hand).fill('#f9f5cb').transform('scale(28.5) translate(10.4 29.86)').id('hand'),
          SVG.use('hand').transform('translate(374 0)')
        ]
      );
  }

  function accesories(uint256 _assetId) public pure returns (SVG.Element memory) {
    Randomizer.Seed memory seed = Randomizer.Seed(_assetId, 0);
    bytes memory bird = 'M4 0h1v2h1v1H5v2H2v1H1V5h1V4H1V3H0V2h3V1h1zm1 2H4v1h1z';
    string[4] memory colors = ['pink', 'orange', 'yellow', 'white'];
    uint256 color;
    (seed, color) = seed.random(colors.length);

    string[5] memory distances = ['0', '7.5', '15', '22.5', '30'];
    string memory transform = string(abi.encodePacked('scale(28.5) translate(', distances[_assetId % 5], ' 0)'));
    SVG.Element memory birdEle = SVG.path(bird).fill(colors[color]).transform(transform).id('bird');

    // imageType:0 => UFO, 1=>Plane other=>Nothing
    uint256 imageType;
    (seed, imageType) = seed.random(50); // percentage

    if (_assetId != 4 || imageType > 1) {
    // if (false) {
      return birdEle;
    } else if (imageType == 0) {
      //return bird and ufo
      SVG.Element memory ufo = SVG
        .group(
          SVG.list(
            [
              SVG.path('M1 0h2v1H1zm0 3h1v1H1zm2 0h1v1H3z').fill('#fb4694'),
        ...
        .transform('scale(28.5) translate(1 1)')
        .id('ufo');
      ...
      SVG.Element memory plane = SVG
        .group(
          SVG.list(
            [
              SVG.path('M0 0h1v1H0zm2 0h2v1h1v1H3V1H2z').fill('#9cb4b8'),
  ...
  function generateSVGPart(uint256 _assetId) public view override returns (string memory svgPart, string memory tag) {
    ...
    (stack.svg, stack.idNouns) = nounsProvider.getNounsSVGPart(nounsId[stack.nounsId]);
    stack.svgNouns = SVG.element(bytes(stack.svg));
    stack.nouns = SVG.group(
      [
        background(_assetId),
        accesories(_assetId),
        SVG.use(stack.idNouns).transform('scale(0.9) translate(72.2 113)'),
        hands()
      ]
    );
